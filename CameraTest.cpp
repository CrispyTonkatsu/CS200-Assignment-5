...
const glm::vec4 O = cs200::point(0, 0), EX = cs200::vector(1, 0), EY = cs200::vector(0, 1), BLACK(0, 0, 0, 1),
                BLUE(0, 0, 1, 1), WHITE(1, 1, 1, 1), RED(1, 0, 0, 1);

namespace {
  // random floating point numbers
  float frand(float a = 0, float b = 1) { return a + (b - a) * float(rand()) / float(RAND_MAX); }
} // namespace
...
Client::Client(void) {
  ...
  for (int i = 0; i < 200; ++i) {
    float w = frand(0.5f, 2), h = frand(0, 5);
    glm::mat4 M = cs200::translate(frand(-50, 50) * EX + 0.5f * h * EY) * cs200::scale(0.5f * w, 0.5f * h);
...
Client::~Client(void) { render.unloadMesh(); }
...
void Client::draw(double dt) {
  ...
  square_map = cs200::rotate(square_rrate * dt) * square_map;
  ...
  if (!moving_mode) {
    obj2ndc = world2ndc * cs200::cameraToWorld(camera1) * cs200::NDCToCamera(camera1);
    render.setTransform(obj2ndc);
...
void Client::keypress(SDL_Keycode kc) {
  const float distance_increment = 0.05f, angle_increment = 360.0f / 100.0f, zoom_increment = 0.95f;
  switch (kc) {
    case SDLK_d: camera1.moveRight(distance_increment * camera1.height()); break;
    case SDLK_a: camera1.moveRight(-distance_increment * camera1.height()); break;
    case SDLK_w: camera1.moveUp(distance_increment * camera1.height()); break;
    case SDLK_x: camera1.moveUp(-distance_increment * camera1.height()); break;
    case SDLK_s: camera1.rotate(angle_increment); break;
    case SDLK_1: camera1.zoom(zoom_increment); break;
    case SDLK_2: camera1.zoom(1.0f / zoom_increment); break;
    case SDLK_SPACE: moving_mode = !moving_mode; break;
    default: camera1 = cs200::Camera(O, EY, aspect * 5, 5);
  }
}
...
void Client::resize(int W, int H) {
  ...
  camera0 = cs200::Camera(camera0.center(), camera0.up(), aspect * camera0.height(), camera0.height());
  camera1 = cs200::Camera(camera1.center(), camera1.up(), aspect * camera1.height(), camera1.height());
}
...
int main(int , char *[]) {
  ...
  SDL_Window *window = SDL_CreateWindow(
      title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
      ...
      while (SDL_PollEvent(&event)) {
        switch (event.type) {
          ...
          case SDL_WINDOWEVENT:
            if (event.window.event == SDL_WINDOWEVENT_RESIZED) client->resize(event.window.data1, event.window.data2);
            break;
        }
      }
